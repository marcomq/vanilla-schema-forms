...
const advancedOptionsRenderer = {
  render: (node, path, elementId, dataPath, context) => {
    ...
    if (node.type !== "object") {
      if (node.type === "string")
        return domRenderer.renderString(node, elementId, getName(dataPath));
      if (node.type === "boolean")
        return domRenderer.renderBoolean(node, elementId, getName(dataPath));
      if (node.type === "number" || node.type === "integer") {
        const safeNode =
          node.defaultValue === null ? { ...node, defaultValue: "" } : node;
        return domRenderer.renderNumber(safeNode, elementId, getName(dataPath));
      }
